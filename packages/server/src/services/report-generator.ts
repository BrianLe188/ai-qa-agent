// ============================================================
// Report Generator — Beautiful HTML Test Reports
// ============================================================
...
function generateStepRows(steps: StepResult[]): string {
  return steps
    .map(
      (step) => `
    <tr class="step-row step-${step.status}">
      ...
      <td>${step.screenshotPath ? `<a href="/api/screenshots/${basename(step.screenshotPath)}" target="_blank">📸</a>` : ""}</td>
    </tr>`,
    )
    .join("\n");
}
...
function generateTestCaseCard(result: TestCaseResult, index: number): string {
  const screenshotHtml = result.screenshotPath
    ? `<div class="failure-screenshot">
        <h4>📸 Failure Screenshot</h4>
        <img src="/api/screenshots/${basename(result.screenshotPath)}" alt="Failure screenshot" />
       </div>`
    : "";
    ...
    <div class="test-case-card" id="tc-${index}">
      ...
      <div class="tc-details" id="details-${index}">
        <table class="steps-table">
          ...
          <tbody>
            ${generateStepRows(result.stepResults)}
          </tbody>
        </table>
        ${screenshotHtml}
        ${analysisHtml}
...
export function generateHtmlReport(run: TestRun, planName?: string): string {
  ...
  const testCaseCards = run.results
    .map((r, i) => generateTestCaseCard(r, i))
    .join("\n");
